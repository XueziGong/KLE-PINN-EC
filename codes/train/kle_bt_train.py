...
def loop(DEVICE, mu_lnKs, eta, L, variance, nkl, exp_design, random_seed):
    ...
    eigen_values = kle_pinn_bt.eigenvalue(eta, variance, wn).reshape(-1,1)
    ...
    # build the model
    pinn = kle_pinn_bt.pinn(nkl, eta, variance, wn, L, DEVICE).to(DEVICE)
    ...
    def compute_loss():
        ...
        Ks_col = kle_pinn_bt.compute_Ks(z_col, xi_pred_col, eigen_values,
                                     mu_lnKs, eta, wn, L)
